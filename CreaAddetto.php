...
<body class="sfondo">
    ...
    <center>
    <div class="wrapper">
        <form action="DB/AggAddetto.php" method="post">
            <h1>Registrazione</h1>
            <div class="input-box via-civico">
                <input type="text" class="via" name="nome" placeholder="Nome" required>
                <input type="date" class="civico" name="dataNascita" placeholder="Data di Nascita" required>
            </div>
            <div class="input-box">
                <label for="idIncarico">Seleziona Incarico:</label>
                <select name="idIncarico" id="idIncarico" required>
                    <option value="">--Seleziona--</option>
                    <?php
                    while ($incarico = mysqli_fetch_assoc($resultIncarichi)) {
                        echo "<option value='" . htmlspecialchars($incarico['IDIncarico']) . "'>" . htmlspecialchars($incarico['Nome']) . "</option>";
                    }
                    ?>
                </select>
            </div>
